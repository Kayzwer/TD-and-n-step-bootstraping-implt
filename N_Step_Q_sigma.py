...
class Agent:
    def __init__(self, alpha: float, gamma: float, epsilon: float,
                 sigma: float, n_actions: int, n_step: int) -> None:
        assert 0. < alpha <= 1.
        assert 0. < gamma < 1.
        assert 0. <= epsilon <= 1.
        assert 0. <= sigma <= 1.
        ...
        self.greedy_policy_action_prob_memory = []
    ...
    def choose_action(self, state: str, valid_actions: List[int]
        ...
        return action, target_action_prob / behaviour_action_prob, target_action_prob
    ...
    def store_info(self, state: Optional[str] = None,
                   ...
                   greedy_policy_action_prob: Optional[float] = None,
        ...
        if greedy_policy_action_prob != None:
            self.greedy_policy_action_prob_memory.append(
                greedy_policy_action_prob)
    ...
    def update(self, done: bool) -> None:
        if len(self.state_memory) == self.n_step + 1:
            ...
            for i in range(self.n_step - 1, -1, -1):
                if i == self.n_step - 1:
                    ...
                    return_sum = self.reward_memory[i] + self.gamma * ((
                        self.sigma * self.importance_sampling_ratio_memory[i] +
                        self.unit_m_sigma *
                        self.greedy_policy_action_prob_memory[i]) * (
                            return_sum - self.state_actions_value[
                                self.state_memory[i]][self.action_memory[i]]) +
            ...
            self.greedy_policy_action_prob_memory.pop(0)
    ...
    def clear_memory(self) -> None:
        ...
        self.greedy_policy_action_prob_memory.clear()
